...
namespace Hutech.Exam.Client.Pages.Exam
{
    public partial class Exam
    {
        ...
        private async Task getListDapAn(long? ma_de_hoan_vi)
        {
            HttpResponseMessage? response = null;
            if (httpClient != null)
                response = await httpClient.GetAsync($"api/Result/GetListDapAn?ma_de_thi_hoan_vi={ma_de_hoan_vi}");
            if (response != null && response.IsSuccessStatusCode && myData != null)
            {
                var resultString = await response.Content.ReadAsStringAsync();
        ...
        private async Task InsertChiTietBaiThi()
        {
            ...
            if (httpClient != null && chiTietCaThi != null && myData != null && myData.chiTietCaThi != null)
                response = await httpClient.GetAsync($"api/Exam/InsertChiTietBaiThi?ma_chi_tiet_ca_thi={chiTietCaThi.MaChiTietCaThi}&ma_de_thi_hoan_vi={myData.chiTietCaThi.MaDeThi}");
